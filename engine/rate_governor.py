...
class RampController:
    ...
    def __init__(self, target_mbps: float, ramp_up_s: int,
                 ramp_down_s: int, duration_s: int) -> None:
    ...
    def get_phase(self, elapsed_s: float) -> str:
        ...
        if elapsed_s < self.ramp_up_s:
            return "ramp_up"
        if elapsed_s >= self.duration_s - self.ramp_down_s:
            return "ramp_down"
        return "steady"
...
class RateGovernor:
    ...
    def __init__(self, global_max_mbps: float,
                 abort_threshold_mbps: float) -> None:
    ...
    def check_node(self, node_id: str, phase_id: str,
                   measured_mbps: float,
                   limit_mbps: float) -> Optional[EnforcementEvent]:
        if measured_mbps > limit_mbps * 1.10:
            evt = EnforcementEvent(
                timestamp=self._now(), node_id=node_id, phase_id=phase_id,
                measured_mbps=round(measured_mbps, 2),
    ...
    def check_aggregate(self, phase_id: str,
                        measurements: Dict[str, float]
                        ) -> Optional[EnforcementEvent]:
        total = sum(measurements.values())
        if total > self.abort_threshold_mbps:
            evt = EnforcementEvent(
                timestamp=self._now(), node_id="AGGREGATE", phase_id=phase_id,
                measured_mbps=round(total, 2),
        ...
        if total > self.global_max_mbps:
            evt = EnforcementEvent(
                timestamp=self._now(), node_id="AGGREGATE", phase_id=phase_id,
                measured_mbps=round(total, 2),
                limit_mbps=round(self.global_max_mbps, 2),
                action="global_throttle",
            )
            self.enforcement_log.append(evt)
            return evt
        return None
