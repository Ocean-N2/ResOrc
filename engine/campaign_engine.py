"""
CampaignEngine – orchestrates the full lifecycle of a resilience-test campaign.
"""
...
from config.models import (
    AttackMode, CampaignConfig, MetricsSnapshot, NodeRole,
    PhaseConfig, PhaseStatus, RateConfig,
)
...
class CampaignEngine:
    ...
    def __init__(self, campaign: CampaignConfig, node_manager: NodeManager,
        ...
        self.governor = RateGovernor(
            campaign.global_max_rate_mbps, campaign.abort_threshold_mbps)
        self._aborted = False
    ...
    def pre_flight(self) -> Dict[str, Any]:
        ...
        target_summary = self.tm.get_summary()
        return {
            "node_health": health, "config_warnings": warnings,
    ...
    async def run_phase(self, phase: PhaseConfig,
        ...
        ramp = RampController(
            target_mbps=phase_mbps, ramp_up_s=phase.ramp_up_seconds,
            ramp_down_s=phase.ramp_down_seconds,
            duration_s=phase.duration_seconds,
        )
        pairs = self.nm.get_nodes_for_phase(phase)
        if not pairs:
            return {"phase_id": phase.phase_id, "status": "skipped",
                    "reason": "no nodes assigned"}
        ...
        for node, ctrl in pairs:
            ...
            if isinstance(ctrl, TrexController):
                ...
                ctrl.start(phase.attack_mode, phase.targets,
                           node_rate, phase.duration_seconds)
        ...
        while True:
            ...
            if ramp_phase == "steady":
                status = PhaseStatus.ACTIVE
            elif ramp_phase == "ramp_down":
                status = PhaseStatus.COOLING
            ...
            for node, ctrl in pairs:
                ...
                snap = MetricsSnapshot(
                    timestamp=now_iso, node_id=node.node_id,
                    phase_id=phase.phase_id, tx_mbps=tx,
                ...
                evt = self.governor.check_node(
                    node.node_id, phase.phase_id, tx, per_node_target)
                if evt:
                    enforcement_events.append(evt)
                    throttled_rate = RateConfig(
                        value_str=f"{per_node_target * 0.9}Mbps")
                    if isinstance(ctrl, TrexController):
                        ctrl.update_rate(throttled_rate)
                    elif isinstance(ctrl, MHDDoSController):
                        ctrl.restart_with_threads(
                            throttled_rate.to_mhddos_threads())

            agg_evt = self.governor.check_aggregate(phase.phase_id, node_tx)
            if agg_evt:
                enforcement_events.append(agg_evt)
                if agg_evt.action == "abort":
                    self._aborted = True

            if on_tick:
                on_tick({
                    "elapsed": round(elapsed, 1), "phase_id": phase.phase_id,
        ...
        return {
            ...
            "peak_tx_mbps": round(
                max((s.tx_mbps for s in metrics_history), default=0), 2),
            "avg_tx_mbps": round(
                (sum(s.tx_mbps for s in metrics_history)
                 / max(1, len(metrics_history))), 2),
        }
    ...
    async def run_campaign(self, phase_selector: Optional[List[str]] = None,
        ...
        for phase in phases:
            if self._aborted:
                results.append({"phase_id": phase.phase_id, "status": "aborted"})
                continue
            res = await self.run_phase(phase, on_tick=on_tick)
            results.append(res)
        return {
            "campaign_id": self.campaign.campaign_id,
            "name": self.campaign.name, "pre_flight": pre,
